...
- **Metodología** (`.claude/rules/metodologia-as-code.md`, eje **nuevo**): el método del PRODUCTO (lo que
  Cockpit vende, distinto del proceso de desarrollo) — SSoT = `sistema/metodo/methodologies.yaml` (59 M-cards)
  + `proceso/**` + `sistema/schema/objeto.schema.yaml` (21 entidades) → `gen_metodo.py`.
...
- El objeto normalizado COMPLETO (21 entidades — CK-26 + apuesta D-23 + el gobierno del directorio D-24..D-29:
  `riesgo`·`sesion`·`acuerdo`·`periodo`·`proyeccion_caja`·`presupuesto`·`inversion` + el archivo D-38: `documento`) se lee/valida entero en `/api/objeto` (CK-13, CAP-08;
