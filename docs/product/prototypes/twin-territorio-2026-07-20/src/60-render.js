...
function render(){
  ...
  const tablero=terr&&(state.nivel===1||state.nivel===3);   // v14: niveles 1/3 = tableros pageView
  ...
  backB.disabled = !terr || tablero || state.escala==='z0';
  ...
  backB.title = backB.disabled
    ...
    : 'Volver al mapa completo (sube un nivel de zoom)';
  ...
  document.getElementById('search').disabled=!terr||tablero;
  document.querySelector('.search').style.opacity=(terr&&!tablero)?'':'.4';
  document.querySelector('.search').title=(!terr||tablero)?'La búsqueda actúa sobre el mapa — disponible en el nivel 2 (Estratégico) y dentro de un proceso':'';
  const hintEl=document.getElementById('hint'); if(hintEl) hintEl.style.display=tablero?'none':'';
  ...
  const apl=(terr&&!tablero)?CAPAS_APLICAN[state.escala==='z0'?('z0-'+state.piel):state.escala]:[];
  ...
  if(terr&&state.nivel===3) crumb='<b>Nivel 3 · Táctico</b> — el compromiso entre áreas';
