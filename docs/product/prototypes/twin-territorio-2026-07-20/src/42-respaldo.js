/* ===== v14.3 · capa RESPALDO DEL MÉTODO — cada panel cita la metodología que lo sostiene =====
   ...
   sus fuentes igual que todo lo demás. SSoT real = sistema/metodo/methodologies.yaml (54 cartas).
...
const RESPALDO={
  ...
  'ent-empresa':{ cita:'ontología del twin', t:'Empresa — el twin completo', items:[
    {f:'Ontología del twin — semántica + acción', uso:'las 13 entidades y sus relaciones son una gramática declarada; cada cosa que ves es una instancia con su ficha.', m:'M32'},
...
function openRespaldo(id){ const r=RESPALDO[id]; if(!r) return;
  openDrawer('Respaldo del método', r.t,
    ...
    + `<div style="font-size:11px;color:var(--tx-faint);border-top:1px solid var(--border);padding-top:8px;margin-top:4px">El método es dato versionado (54 cartas) — la misma fuente que respalda cada campo del twin y cada paso del engagement. <span class="plnk" onclick="state.mod='metodo';state.insp='home';render()">Ver la trazabilidad completa — módulo Método ›</span></div>`); }
